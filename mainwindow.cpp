...
void MainWindow::on_pb_ajouter_mat_clicked()
{
...
void MainWindow::on_pb_supprimer_mat_clicked()
{
    Materiel M1; M1.setcode(ui->le_code_supp->text().toInt());
    bool test=M1.supprimer(M1.getcode());
    QMessageBox msgBox;
    if(test)
...
void MainWindow::on_pb_mod_mat_clicked()
{
    int code= ui->le_code_mod->text().toInt();
        int prix = ui->le_prix_mod->text().toInt();
                ...
                if(test)
                {
                    ...
                    ui->tab_materiel->setModel(M.afficher());//refresh
                           ...
                           QMessageBox::information(nullptr, QObject::tr("effectuer"),
                                QObject::tr(" Modifier.\n"
                                            "Click Cancel to exit."), QMessageBox::Cancel);
                           ui->le_code_mod->clear();
                           ui->le_prix_mod->clear();
                           ui->le_type_mod->clear();

               }
                else
                {
                    QMessageBox::critical(nullptr, QObject::tr("non effectuer"),
                                QObject::tr("non modifier !.\n"
                                            "Click Cancel to exit."), QMessageBox::Cancel);
                    ui->le_code_mod->clear();
                    ui->le_prix_mod->clear();
                    ui->le_type_mod->clear();
...
void MainWindow::on_lienEdit_recherche_mat_textChanged()
{
    QString rech=ui->lineEdit_recherche->text();
    ui->tab_materiel->setModel(M.RechercheMateriel(rech));

}

void MainWindow::on_trier_mat_clicked()
{
    {
        if(ui->comboBox->currentText()=="trier par code")
        {
            ui->tab_materiel->setModel(M.triermaterielparcode());

        }
        else if(ui->comboBox->currentText()=="trier par prix")
        {
            ui->tab_materiel->setModel(M.triermaterielparprix());
...
void MainWindow::on_pdfexport_mat_clicked()
{
    QSqlQuery query;
